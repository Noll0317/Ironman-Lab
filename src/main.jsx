...
function App(){
  ...
  return <div className="app">
    ...
    <nav>{[['dashboard','dashboard'],['calendar','calendar'],['plan','add plan'],['log','log actual'],['readiness','readiness'],['fuel','fuel lab'],['sweat','sweat calc'],['simulation','race sims'],['predictor','Maryland predictor'],['coach','coach report'],['analysis','AI screenshots'],['experiments','experiments']].map(([v,label])=><button key={v} onClick={()=>setView(v)} className={view===v?'active':''}>{label}</button>)}</nav>
    {loading? <p>Loading...</p> : view==='dashboard'? <Dashboard stats={stats} workouts={workouts} planned={planned} readiness={latestReadiness} deleteWorkout={deleteWorkout} startLogFromPlan={startLogFromPlan}/> : view==='calendar'? <Calendar workouts={workouts} planned={planned} deletePlan={deletePlan} startLogFromPlan={startLogFromPlan}/> : view==='plan'? <Plan form={planForm} setForm={setPlanForm} savePlan={savePlan}/> : view==='log'? <Log form={form} setForm={setForm} saveWorkout={saveWorkout}/> : view==='readiness'? <Readiness form={readyForm} setForm={setReadyForm} saveReadiness={saveReadiness} readiness={readiness}/> : view==='fuel'? <FuelLab workouts={workouts}/> : view==='sweat'? <SweatCalculator/> : view==='simulation'? <RaceSimulations workouts={workouts}/> : view==='predictor'? <MarylandPredictor workouts={workouts}/> : view==='coach'? <CoachReport workouts={workouts} planned={planned} readiness={readiness}/> : view==='analysis'? <ScreenshotAnalysis/> : <Experiments experiment={experiment} setExperiment={setExperiment} saveExperiment={saveExperiment} experiments={experiments}/>} 
  </div>
}
