...
function App(){
  ...
  return <div className="app">
    ...
    <nav>{[['dashboard','dashboard'],['calendar','calendar'],['plan','add plan'],['log','log actual'],['readiness','readiness'],['analysis','AI screenshots'],['experiments','experiments']].map(([v,label])=><button key={v} onClick={()=>setView(v)} className={view===v?'active':''}>{label}</button>)}</nav>
    {loading? <p>Loading...</p> : view==='dashboard'? <Dashboard stats={stats} workouts={workouts} planned={planned} readiness={latestReadiness} deleteWorkout={deleteWorkout} startLogFromPlan={startLogFromPlan}/> : view==='calendar'? <Calendar workouts={workouts} planned={planned} deletePlan={deletePlan} startLogFromPlan={startLogFromPlan}/> : view==='plan'? <Plan form={planForm} setForm={setPlanForm} savePlan={savePlan}/> : view==='log'? <Log form={form} setForm={setForm} saveWorkout={saveWorkout}/> : view==='readiness'? <Readiness form={readyForm} setForm={setReadyForm} saveReadiness={saveReadiness} readiness={readiness}/> : view==='analysis'? <ScreenshotAnalysis/> : <Experiments experiment={experiment} setExperiment={setExperiment} saveExperiment={saveExperiment} experiments={experiments}/>} 
  </div>
}
...
function Calendar({workouts,planned,deletePlan,startLogFromPlan}){ const start=weekStart(); const days=Array.from({length:28},(_,i)=>{const d=new Date(start); d.setDate(start.getDate()+i); return d;}); return <main><section className="panel"><h2><CalendarDays/> Next 4 Weeks</h2><p className="muted">Planned workouts can be logged or deleted from here. Completed workouts appear under Done.</p><div className="week fourWeeks">{days.map(d=>{ const iso=toISO(d); const dayPlans=planned.filter(w=>w.date===iso); const dayLogs=workouts.filter(w=>w.date===iso); return <div className="day" key={iso}><h3>{d.toLocaleDateString(undefined,{weekday:'short'})}<span>{iso}</span></h3>{dayPlans.map(p=><div className={`mini plannedMini ${p.completed?'done':''}`} key={p.id}><div><b>Plan:</b> {p.type} — {p.title}</div><div className="miniActions">{!p.completed&&<button onClick={()=>startLogFromPlan(p)}>log</button>}<button className="danger" onClick={()=>deletePlan(p.id)}>delete</button></div></div>)}{dayLogs.map(w=><div className={`mini ${w.type}`} key={w.id}><b>Done:</b> {w.type}: {w.title||`${w.distance} ${TYPES[w.type]?.unit}`}</div>)}</div>})}</div></section></main> }
...
function ScreenshotAnalysis(){
  ...
  return <main>
    <section className="panel">
      ...
      <p className="muted">Drop Garmin screenshots here first. This version saves the workflow and preview area. True AI extraction is the next upgrade and will need a secure backend/API key so the screenshots can be analyzed safely.</p>
      <label className="dropZone">
        <UploadCloud size={34}/>
        <strong>Choose Garmin screenshots</strong>
        <span>Charts, laps, overview, HR zones, pace, temp, nutrition notes</span>
        <input type="file" multiple accept="image/*" onChange={e=>setFiles(Array.from(e.target.files||[]))}/>
      </label>
      <label className="wide analysisNotes">Workout notes / what you want analyzed<textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Example: Long brick. Check HR drift, heat impact, gut, sodium/fluid, pacing, and what to adjust next time."/></label>
      ...
      <div className="aiBox"><h3>Next AI build</h3><p>Auto-read distance, duration, pace/speed, HR, HR zones, temp, laps, and training effect, then create a draft workout log plus coach analysis.</p></div>
    </section>
  </main>
}
